...
'''
   ...
   标签为png图片，图片为jpg格式，无需固定大小，传入训练前会自动进行resize。
   数据集遵循VOC2007格式
...
if __name__ == "__main__":
    ...
    # -----------------------------------8-----------------------------------------------------------------------------------------#
    model_path = "logs/farm_lake_setr_naive_s_23/best_epoch_weights.pth"
    # ---------------------------------------------------------#
    ...
    root_dataset_path = 'VOCdevkit'
    dataset_path = os.path.join(root_dataset_path,'tif_dataset')
    src_data_path = os.path.join(dataset_path,'src')
    ...
    # model = create_model(num_classes=num_classes, has_logits=True)
    ...
    #   读取数据集对应的txt
    # ---------------------------#
    with open(os.path.join(root_dataset_path, "tif_dataset/srcSets/Segmentation/train.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(root_dataset_path, "tif_dataset/srcSets/Segmentation/val.txt"),"r") as f:
